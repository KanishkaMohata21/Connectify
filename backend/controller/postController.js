...
export const createPost = async (req, res) => {
    try {
        ...
        const newPost = new Post({
            text: text,
            image: img ? {
                data: img, // This should be a base64 string
                contentType: 'image/jpeg' // Assuming jpeg, you can adjust based on your needs
            } : undefined,
            user: user._id,
        });
...
export const allfollowingpost = async (req, res) => {
    try {
        ...
        if (posts.length === 0) {
            return res.status(403).json({ error: "No posts found" });
        }
        return res.status(200).json({ message: "All posts fetched successfully", posts: posts });
